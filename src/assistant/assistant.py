...
from src.core.exceptions import AssistantError, LLMError, SearchError
...
class AssistantService:
    ...
    async def _process_text_query_stream_impl(
        ...
        try:
            ...
            if self.chat_history and session_id:
                ...
                if await self.chat_history.check_summarization_needed(session_id):
                    old_messages = await self.chat_history.get_old_messages_for_summarization(
                        session_id, keep_recent=self.config.memory.max_history_messages, offset=summarized_count
                    )
                    if old_messages:
                        ...
                        if new_delta_summary:
                            ...
                            await self.chat_history.save_summary(
                                session_id, combined_summary, summarized_count + len(old_messages)
                            )
            ...
            if history_messages:
                search_query = await self.text_llm.decontextualize_query(
                    query, history_messages, company_name=company_display_name
                )
    ...
    async def process_voice_query(
        ...
        await self.set_request_active(session_id)
        try:
            result = await self._process_voice_query_impl(
                ogg_bytes, system_prompt, use_rag, limit, session_id, platform, audio_format
            )
            return result
        finally:
            await self.clear_request_active(session_id)
    ...
    async def _process_voice_query_impl(
        ...
        if current_system_prompt is None:
            ...
            sys_template = (
                self.audio_llm.model_config.system_prompt_template if self.audio_llm.model_config else "{context}"
            )

            if use_rag:
                current_system_prompt = f"ТЕКУЩАЯ КОМПАНИЯ: {company_display_name}\nКОНТЕКСТ ДИАЛОГА:\n{history_context}\n\nИНСТРУКЦИИ:\n{tool_template}"
            else:
                current_system_prompt = (history_context or "") + sys_template.format(
        ...
        if use_rag:
            ...
            async def search_callback(query: str) -> List[str]:
                ...
                search_result: SearchResult = await self.search.search(
                    query, self.config.rag.search_limit or 10, company_id=user_company
                )
                ...
                clean_chunks = [
                    re.sub(r"https?://\S+", "[ссылка прикреплена в чате]", chunk) for chunk in search_result.chunks
                ]

                max_chars = (
                    self.audio_llm.model_config.max_voice_context_chars if self.audio_llm.model_config else 50000
                )
            ...
            if self.chat_history and session_id:
                ...
                await self.chat_history.save_message(
                    session_id, platform, "assistant", transcript or "[Голос]", metadata
                )
            ...
            audio_response, transcript = await self.audio_llm.process_voice_from_pcm(
                pcm_data, current_system_prompt, format=audio_format
            )
            if self.chat_history and session_id:
                await self.chat_history.save_message(session_id, platform, "user", "[Голос]", {"voice": True})
                await self.chat_history.save_message(
                    session_id, platform, "assistant", transcript or "[Голос]", {"voice": True}
                )
            # Синхронизируем голосовую историю с LangGraph-памятью оркестратора
    ...
    async def create_helpdesk_ticket(
        self, query: str, assistant_answer: str, session_id: Optional[str] = None, **kwargs
    ) -> TicketCreationResult:
        summary = None
        try:
            history_messages = (
                await self.chat_history.get_history(session_id) if self.chat_history and session_id else []
            )
            dialog = (
                "\n".join(
                    [
            ...
            summary = await self.text_llm.generate(
                f"Кратко опиши суть проблемы для тикета на основе диалога:\n{dialog}", temperature=0.1
            )
        except:
            pass
