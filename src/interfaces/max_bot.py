...
async def get_cloudflared_quicktunnel_url() -> Optional[str]:
    """Автоматически получает URL временного туннеля из metrics API cloudflared."""
    import aiohttp
    # Пытаемся опросить метрики на хостах: cloudflared (внутри docker), 127.0.0.1 (локально)
    for host in ["cloudflared", "127.0.0.1", "localhost"]:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"http://{host}:4567/quicktunnel", timeout=3) as resp:
                    if resp.status == 200:
...
async def run_max_bot(config: Config, assistant: Optional[AssistantService] = None):
    ...
    webhook_url = config.max_webhook_url
    if webhook_url and webhook_url.lower() == "auto":
        logger.info("🔄 Автоматическое определение URL Cloudflare туннеля...")
        tunnel_url = await get_cloudflared_quicktunnel_url()
        if tunnel_url:
            webhook_url = f"{tunnel_url}/webhook/max"
            logger.info(f"✅ Найден туннель: {tunnel_url}")
        else:
            logger.error("❌ Не удалось получить URL от cloudflared (проверьте наличие --metrics 0.0.0.0:4567)")
            webhook_url = None

    if webhook_url:
        logger.info(f"🔗 MAX бот: включен режим Webhook. Установка URL: {webhook_url}")
        success = await client.set_webhook(webhook_url, secret=config.max_webhook_secret)
        if success:
